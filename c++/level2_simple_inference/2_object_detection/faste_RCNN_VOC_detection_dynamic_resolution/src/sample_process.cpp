...
Result SampleProcess::MainProcess(string input_path)
{
    ...
    const char* omModelPath = "../model/faster_rcnn.om";
    ...
    ret = processModel.CreateImageInfoBuffer();
    if (ret != SUCCESS) {
        ERROR_LOG("Create Image InfoBuffer failed");
        return FAILED;
    }
    ...
    for (string path : file_vec) {
        ...
        cout << "ready to preprocess" << endl;
        if (!Utils::PreProcess(image_desc, path)) {
            continue;
        }

        cout << "ready to SetImageInfoBuffer" << endl;
        ret = processModel.SetImageInfoBuffer();
        if (ret != ACL_ERROR_NONE) {
            ERROR_LOG("Set Image Info Buffer failed");
            return FAILED;
        }
        INFO_LOG("Set Image Info Buffer success");

        // create input for the model inference.
        cout << "ready to CreateInput" << endl;
