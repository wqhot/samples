...
bool Utils::PreProcess(shared_ptr<ImageDesc>& imageData, const string& imageFile) {
    ...
    uint32_t size = IMAGE_CHAN_SIZE_U8(MODEL_INPUT_WIDTH, MODEL_INPUT_HEIGHT);
    ...
    if (runMode_ == ACL_HOST) {
        imageDev = CopyDataHostToDevice(reiszedImage.ptr<u_int8_t>(0), size);
        if (imageDev == nullptr) {
        ...
        imageDev = CopyDataDeviceToDevice(reiszedImage.ptr<u_int8_t>(0), size);
        if (imageDev == nullptr) {
            ERROR_LOG("Copy image info to device failed");
            return FAILED;
        }
