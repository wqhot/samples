#!/bin/bash
script_path="$HOME/sample-classification-python/classificationapp/models"
model_path="$HOME/sample-classification-python/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    ...
    C31_flag=`find $HOME -maxdepth 1 -name "sample-classification-python" 2> /dev/null`
    if [[ $C31_flag ]];then
        ...
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            ...
            echo "[INFO] Please manually delete $HOME/sample-classification-python director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/sample-classification-python
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed, Please check your environment"
        return 1
    fi

    git clone https://gitee.com/Atlas200DK/sample-classification-python.git $HOME/sample-classification-python
    if [[ $? -ne 0 ]];then
