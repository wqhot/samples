#!/bin/bash
script_path="$HOME/sample-headposeestimation-python/model"
model_path="$HOME/sample-headposeestimation-python/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    ...
    C31_flag=`find $HOME -maxdepth 1 -name "sample-headposeestimation-python" 2> /dev/null`
    if [[ $C31_flag ]];then
        ...
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            ...
            echo "[INFO] Please manually delete $HOME/sample-headposeestimation-python director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/sample-headposeestimation-python
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed, Please check your environment"
        return 1
    fi

    git clone https://gitee.com/Atlas200DK/sample-headposeestimation-python.git $HOME/sample-headposeestimation-python
    if [[ $? -ne 0 ]];then
