...
main()
{
	...
	mv $HOME/AscendProjects/samples/sample-objectdetection $HOME/AscendProjects/
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Git clone failed. Please check your network connection."
        return 1
    fi

    download "faster_rcnn" "1"

    echo "[INFO] Please follow the link from the 编译 step in Readme to continue running the case.(https://gitee.com/ascend/samples.git)"
}
main
